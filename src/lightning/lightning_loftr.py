...
class PL_LoFTR(pl.LightningModule):
    ...
    def optimizer_step(self, epoch, batch_idx, optimizer, optimizer_idx,
        ...
        if self.trainer.global_step < warmup_step:
            if self.config.TRAINER.WARMUP_TYPE == 'linear':
                ...
                lr = base_lr + (self.trainer.global_step / self.config.TRAINER.WARMUP_STEP) * abs(
    ...
    def _trainval_inference(self, batch):
        with self.profiler.profile("Compute coarse supervision"):
            compute_supervision_coarse(batch, self.config)  # 创建粗匹配时的真值

        with self.profiler.profile("LoFTR"):
            self.matcher(batch)

        with self.profiler.profile("Compute fine supervision"):
            compute_supervision_fine(batch, self.config)  # 创建精匹配时的真值
