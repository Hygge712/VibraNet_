...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit':
            print(f'self.train_data_root: {self.train_data_root}')
            print(f'self.train_npz_root: {self.train_npz_root}')
            print(f'self.train_list_path: {self.train_list_path}')
            print(f'self.train_intrinsic_root: {self.train_intrinsic_path}')
            ...
            if isinstance(self.val_list_path, (list, tuple)):
                ...
                for npz_list, npz_root in zip(self.val_list_path, self.val_npz_root):
                    self.val_dataset.append(
                        self._setup_dataset(
                            self.val_data_root, npz_root, npz_list,
                            self.val_intrinsic_path, mode='val',
                            min_overlap_score=self.min_overlap_score_test,
                            pose_dir=self.val_pose_root))
            else:
                self.val_dataset = self._setup_dataset(self.val_data_root, self.val_npz_root, self.val_list_path,
                                                       self.val_intrinsic_path, mode='val',
                                                       min_overlap_score=self.min_overlap_score_test,
                                                       pose_dir=self.val_pose_root)
    ...
    def _setup_dataset(self,
        ...
        if mode == 'train':
            ...
            # # TODO: debug, set [:2] data
            local_npz_names = local_npz_names[:1]  # 数据量太大，在调试的时候可以适当缩小一些
        else:
    ...
    def _build_concat_dataset(
        ...
        for npz_name in tqdm(npz_names, desc=f'[rank:{self.rank}] loading {mode} datasets',
    ...
    def _build_concat_dataset_parallel(
        ...
        with tqdm_joblib(tqdm(desc=f'[rank:{self.rank}] loading {mode} datasets',
            ...
            if data_source == 'ScanNet':
                ...
                raise NotImplementedError()
                datasets = Parallel(n_jobs=math.floor(len(os.sched_getaffinity(0)) * 0.9 / comm.get_local_size()))(
                    delayed(lambda x: _build_dataset(MegaDepthDataset, data_root, osp.join(npz_dir, x),
                                                     mode=mode,
                                                     min_overlap_score=min_overlap_score,
                                                     img_resize=self.mgdpt_img_resize,
                                                     df=self.mgdpt_df,
    ...
    def val_dataloader(self):
        ...
        if not isinstance(self.val_dataset, abc.Sequence):
            ...
            sampler = RandomSampler(self.val_dataset)  # 限制数据加载到数据集自己的采样器
            return DataLoader(self.val_dataset, sampler=sampler, **self.val_loader_params)
        else:
            dataloaders = []
            for dataset in self.val_dataset:
                # sampler = DistributedSampler(dataset, shuffle=False)
                sampler = RandomSampler(dataset)  # 限制数据加载到数据集自己的采样器
