...
class MegaDepthDataset(Dataset):
    def __init__(self,
        ...
        self.scene_info = dict(np.load(npz_path, allow_pickle=True))  # 更改后的从npz文件中获取“标签”数据。
        self.scene_info = dict(np.load(npz_path, allow_pickle=True))  # 更改后的从npz文件中获取“标签”数据。
        self.pair_infos = self.scene_info['pair_infos'].copy()
        del self.scene_info['pair_infos']
        self.pair_infos = [pair_info for pair_info in self.pair_infos if pair_info[1] > min_overlap_score]  # 关键点匹配对
    ...
    def __getitem__(self, idx):  # 重写Dataloader获取样本
        ...
        # for LoFTR training
        if mask0 is not None:  # img_padding is True. 如果是训练场景，就将掩码设置到和卷积之后的尺寸大小, 本文设置的是原尺寸的1/8
