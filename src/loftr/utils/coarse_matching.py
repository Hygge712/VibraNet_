...
class CoarseMatching(nn.Module):
    ...
    def forward(self, feat_c0, feat_c1, data, mask_c0=None, mask_c1=None):
        ...
        if self.match_type == 'dual_softmax':
            ...
            conf_matrix = F.softmax(sim_matrix, 1) * F.softmax(sim_matrix, 2)  # 获得置信度最大的像素点位置
    ...
    def get_coarse_match(self, conf_matrix, data):
        ...
        # 2. mutual nearest (Coarse matching)  对应论文中的mutual nearest neighbor matching
        mask = (mask *
                (conf_matrix == conf_matrix.max(dim=2, keepdim=True)[0]) *  # 结果为0 或 1
                (conf_matrix == conf_matrix.max(dim=1, keepdim=True)[0]))  # 解释：找到最有可能匹配到的一些点(粗匹配)

        # 3. find all valid coarse matches
        # this only works when at most one `True` in each row
        mask_v, all_j_ids = mask.max(dim=2)  # 返回值和索引号
        b_ids, i_ids = torch.where(mask_v)  # i_ids: 第一幅图中哪些点能够匹配上的索引
        j_ids = all_j_ids[b_ids, i_ids]  # j_ids: 第二幅图中哪些点能够匹配上的索引
